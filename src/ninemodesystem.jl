...
function (eq::NineModeSystem)(t::Real, u::AbstractVector, dudt::AbstractVector)
    ...
    @inbounds begin
        ...
        adot[6] = (5*sqrt23*(cαcα - cγcγ)*u[2]*u[4])/(3*sqrtcαcαpluscγcγ) - (2*sqrt23*cαcβcγ*u[3]*u[5])/(sqrtcαcαpluscγcγ*sqrtcβcβpluscγcγ) - ((3*cαcα + 4*cβcβ + 3*cγcγ)*u[6])/(3/invRe) + (cα*u[1]*u[7])/sqrt6 + (sqrt32*cβcγ*u[1]*u[8])/sqrtcαcαpluscβcβpluscγcγ + (cα*u[7]*u[9])/sqrt6 + (sqrt32*cβcγ*u[8]*u[9])/sqrtcαcαpluscβcβpluscγcγ
